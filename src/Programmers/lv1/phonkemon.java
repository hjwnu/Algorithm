...
public class phonkemon {
    public static void main(String[] args) {
        int[] nums1 = new int[]{1,2,23,5,23,1,1,23,1,414,1,51,61,11,12,1,1,1,1,1,1,1};
        System.out.println("총 " +nums1.length + "마리 중 최대 " + solution(nums1)+"종류");
    }
    public static int solution(int[] nums) {
        List<Integer> distinct = new ArrayList<>();
        for(int i =0; i < nums.length;i++){
            if(distinct.contains(nums[i])){continue;}
            distinct.add(nums[i]);
        }
        int ans = nums.length;
        return ans/2 >= distinct.size() ? distinct.size(): ans/2;
    }
}
